<?php
	...
	$conn = oci_connect("SQLADMIN","SQLADMIN", $dbZeus);
		...
		switch($Command)
		{
			...
			case "xmlData":
				if (isset ($_REQUEST["Project"]) && isset ($_REQUEST["Object"]))
				{
					...
					$temp = $_REQUEST["grupoDownLoad"];

					if (strlen($temp) == 0)
					{
						$sqlGrupo = "Select grupo_descarga 
									from vc_grupo_descarga
									where empresa = $empresaLogin
									and   promotor = $usrCode";
						
						$rsDos = oci_parse($conn, $sqlGrupo);
						
						$res = oci_execute($rsDos);
						
						$GrupoDL = "(";
						
						while ($row = oci_fetch_array($rsDos, OCI_NUM))
						{
							$GrupoDL .= $row[0] . ",";
						}
						...
						//$GrupoDL = str_replace(",)", ",0)", $GrupoDL);
					}
					else
						$GrupoDL = "(" . $_REQUEST["grupoDownLoad"] . ")";
